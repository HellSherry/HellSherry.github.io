...
<!--        {{topList}}-->
        <t-layout>
          <t-aside style="height: 86vh;border-right: 1px solid var(--td-text-color-primary)">
            <div>
              <img src="//pic.sherry.games/assets/image/1.png" style='width: 92%;margin-top: 30%;margin-bottom: 20%'
                   class="report_img">
              <br>
...
export default {
  ...
  async created() {
    try {
      const response = await axios1.get("/getReportByNumber?number="+this.Number);
    ...
    // try {
    //   const response = await axios1().get("/getFillAllTwo", {
    //     params: {
    //       groupId: this.Number,
    //       password: 1,
    ...
    //
    // try {
    //   const response = await axios1.get("/getFillAnswersAll", {
    //     params: {
    //       number: this.Number,
    //       password: 1,
    //       user: this.loginUser.sub.username
