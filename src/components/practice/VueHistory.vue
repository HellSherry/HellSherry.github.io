...
<!--        </div>-->
      <t-list style="height: 52vh" size="small" :split="true">
        <t-list-item v-for="(item, index) in topicList">
          <t-list-item-meta image="//pic.sherry.games/assets/image/试卷.png" :title=" item.gname+'(做'+ item.count+'题/共'+item.answers+'题)'"
...
export default {
  ...
  ,
    ...
    async created() {
        try {
            const response = await axios1.get("/getQuestionBank",{
                params: {
                    // user: this.user,
          ...
          console.log(response)
            this.FillAnswers = response.data.result;
        ...
        try {
            const response = await axios1.get("/FillAnswersByNumber?userId="+this.loginUser.sub.userId,);
