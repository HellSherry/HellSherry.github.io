...
width: 100px;
      ...
      <t-menu-item @click="$router.push('/chat')" value="chat"> 光荣榜</t-menu-item>
      ...
      <template #operations>
        ...
        <t-badge v-if="store.loginUser!=null" size="medium" :color="store.loginUser.sub.online?'#00A870': 'red'"
          ...
          <t-button @click="$router.push('/my')" variant="text" shape="square">
            <t-avatar  v-bind:image=" store.loginUser.sub.userImg=='no'?'': ('//pic.sherry.games/assets/userAvatar/'+store.loginUser.sub.userImg)" size="small">{{ store.loginUser.sub.username.substring(0, 1) }}</t-avatar>
