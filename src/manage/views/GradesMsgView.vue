...
function openView(row) {
  // window.open(`//pic.sherry.games/assets/paperImg/${ row.paperid}.png`, '_blank', 'width=1000,top=100,left=100')

  window.open( '','','width=600 ,height=2000,top=100,left=600,location=no,menubar=no,status=no,titlebar=no,toolbar=no')
      .document.write(`<img style="width: 100%" src='//pic.sherry.games/assets/paperImg/${ row.paperid}.png'/>`);
}
watch(selectValue,()=>{
  selectGrades()
    }
...
<template>
  <div style="position: relative;height: 102vh">
    ...
    <t-drawer destroy-on-close v-model:visible="drawerVisible"  size="520"  >
      <template #header>
        <h3 style="position: relative;width: 100%"> 成绩详情

          <t-select   v-model="selectValue"  style="position: absolute;margin-top: -24px;right: 0;width: 200px">
