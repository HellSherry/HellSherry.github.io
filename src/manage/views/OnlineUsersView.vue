...
<template>
  ...
  details: [{required: true, message: '内容不能为空'}]
    ...
    <div style="background-position: center"
         :style="`background-image: url('${store.darkT? `//pic.sherry.games/assets/image/mobileback/th (${Math.round(Math.random()*30)}).jpg`:`//pic.sherry.games/assets/image/mobileback/th (${Math.round(Math.random()*30)}).jpg`}');background-repeat: no-repeat;background-size: cover;min-height: 46vh ;margin-top: -20px`">
      ...
      <t-table
          ...
          :columns="[
           { colKey: 'row-select', type: 'multiple' ,width: 60,},
            ...
            { colKey: 'userEmail', title: 'userEmail',ellipsis: true,},
            // { colKey: 'paperDescription', title: 'paperDescription'},
            { colKey: 'username', title: 'username'},
            { colKey: 'grade', title: 'grade'},
            { colKey: 'userImg', title: 'userImg'},
            { colKey: 'defaultAddress', title: 'defaultAddress'},
            { colKey: 'identity', title: 'identity'},
