        ...
        <t-card style="height: 82vh" v-if="menuItemActive==='回复我的'" size="small"  >
          ...
          <t-list style="max-height: 80vh" :scroll="{ type: 'virtual' }" :split="true">
            <t-list-item @click="$router.push('/ask/'+item.askId)" v-for="(item, index) in askDetail" :key="index">
              <template #content>
                <t-comment :avatar=" item.userImg=='no'?'': ('//pic.sherry.games/assets/userAvatar/'+(item.userImg==null?'-1.png':item.userImg))" :author="item.username" :datetime="dayjs(item.creatTime).format('YYYY-MM-DD HH:mm:ss')  " >
                  <template #actions>
